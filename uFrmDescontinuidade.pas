...
begin
  ...
  begin
    Close;
    Comando := 'select b.id,b.numbico,b.data_movto,b.volume,p.id as codpro, ' + '       p.descricao from bico_perdas b ' +
      'inner join bicos_abastec x on x.bico_concentrador = b.numbico ' + 'inner join tanques_abastec t on x.id_tanque = t.id ' +
      'inner join produtos p on p.id = t.id_produto ' + 'where b.pendente = ' + QuotedStr('S') + ' ';
    if sIDProd > 0 then
...
procedure TFrmDescontinuidade.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #27 then
  begin
