...
uses
  ...
  ACBrSpedFiscal, ACBrEFDBlocos, ACBrUtil, ACBrTXTClass, DB, DBClient, 
  ACBrSpedPisCofins, uDmAcBr, uLibrary;
...
uses
  ACBrEFDBloco_H, ACBrEFDBloco_C, ACBrEFDBloco_1, ACBrEFDBloco_1_Class, 
  ACBrEFDBloco_C_Class, FireDAC.Stan.Intf, FireDAC.Stan.Option, 
...
var Q1300, Q1310, Q1320, Q1350, Q1360, Q1370, QTANQ : TFDQuery;
begin
  Q1300 := TFDQuery.Create(nil);
  Q1310 := TFDQuery.Create(nil);
  Q1320 := TFDQuery.Create(nil);
  Q1350 := TFDQuery.Create(nil);
  Q1360 := TFDQuery.Create(nil);
  Q1370 := TFDQuery.Create(nil);
  QTANQ := TFDQuery.Create(nil);
  try
    Q1300.Connection := ConexaoDados;
    Q1310.Connection := ConexaoDados;
    Q1320.Connection := ConexaoDados;
    Q1350.Connection := ConexaoDados;
    Q1360.Connection := ConexaoDados;
    Q1370.Connection := ConexaoDados;
    QTANQ.Connection := ConexaoDados;
    // Alimenta o componente com informações para gerar todos os registros do
    // Bloco 1.
...
var AuxStr, aUF, aSitTribIPI, aSitTribPis, aSitTribCofins, aCST : String;
    ...
    QNota,QItem,QTrib : TFDQuery;
    Q300,Q310,Q320,Q321, Q350,Q370,Q390 : TFDQuery;
    Q400,Q405,Q420, Q425,Q460,Q470,Q490 : TFDQuery;
...
begin
  ...
  CdsRegistroC490 := TClientDataSet.Create(nil);
   ...
   //
   QNota := TFDQuery.Create(nil);
   QItem := TFDQuery.Create(nil);
   QTrib := TFDQuery.Create(nil);
   Q300 := TFDQuery.Create(nil);
   Q310 := TFDQuery.Create(nil);
   Q320 := TFDQuery.Create(nil);
   Q321 := TFDQuery.Create(nil);
   Q350 := TFDQuery.Create(nil);
   Q370 := TFDQuery.Create(nil);
   Q390 := TFDQuery.Create(nil);
   Q400 := TFDQuery.Create(nil);
   Q405 := TFDQuery.Create(nil);
   Q420 := TFDQuery.Create(nil);
   Q425 := TFDQuery.Create(nil);
   Q460 := TFDQuery.Create(nil);
   Q470 := TFDQuery.Create(nil);
   Q490 := TFDQuery.Create(nil);
   QNota.Connection := ConexaoDados;
   QItem.Connection := ConexaoDados;
   QTrib.Connection := ConexaoDados;
   Q300.Connection := ConexaoDados;
   Q310.Connection := ConexaoDados;
   Q320.Connection := ConexaoDados;
   Q321.Connection := ConexaoDados;
   Q350.Connection := ConexaoDados;
   Q370.Connection := ConexaoDados;
   Q390.Connection := ConexaoDados;
   Q400.Connection := ConexaoDados;
   Q405.Connection := ConexaoDados;
   Q420.Connection := ConexaoDados;
   Q425.Connection := ConexaoDados;
   Q460.Connection := ConexaoDados;
   Q470.Connection := ConexaoDados;
   Q490.Connection := ConexaoDados;
...
//                          AuxValor         := QItem.FieldByName('val_custo').AsFloat;
                          ...
                          {case StrToInt(aSitTribPis) of
                            01 : CST_PIS := pisValorAliquotaNormal;
                            02 : CST_PIS := pisValorAliquotaDiferenciada;
                            03 : CST_PIS := pisQtdeAliquotaUnidade;
                            04 : CST_PIS := pisMonofaticaAliquotaZero;
                            05 : CST_PIS := pisValorAliquotaPorST;
                            06 : CST_PIS := pisAliquotaZero;
                            07 : CST_PIS := pisIsentaContribuicao;
                            08 : CST_PIS := pisSemIncidenciaContribuicao;
                            09 : CST_PIS := pisSuspensaoContribuicao;
                            49 : CST_PIS := pisOutrasOperacoesSaida;
                            50 : CST_PIS := pisOperCredExcRecTribMercInt;
                            51 : CST_PIS := pisOperCredExcRecNaoTribMercInt;
                            52 : CST_PIS := pisOperCredExcRecExportacao;
                            53 : CST_PIS := pisOperCredRecTribNaoTribMercInt;
                            54 : CST_PIS := pisOperCredRecTribMercIntEExportacao;
                            55 : CST_PIS := pisOperCredRecNaoTribMercIntEExportacao;
                            56 : CST_PIS := pisOperCredRecTribENaoTribMercIntEExportacao;
                            60 : CST_PIS := pisCredPresAquiExcRecTribMercInt;
                            61 : CST_PIS := pisCredPresAquiExcRecNaoTribMercInt;
                            62 : CST_PIS := pisCredPresAquiExcExcRecExportacao;
                            63 : CST_PIS := pisCredPresAquiRecTribNaoTribMercInt;
                            64 : CST_PIS := pisCredPresAquiRecTribMercIntEExportacao;
                            65 : CST_PIS := pisCredPresAquiRecNaoTribMercIntEExportacao;
                            66 : CST_PIS := pisCredPresAquiRecTribENaoTribMercIntEExportacao;
                            70 : CST_PIS := pisOperAquiSemDirCredito;
                            71 : CST_PIS := pisOperAquiComIsensao;
                            72 : CST_PIS := pisOperAquiComSuspensao;
                            73 : CST_PIS := pisOperAquiAliquotaZero;
                            74 : CST_PIS := pisOperAqui_SemIncidenciaContribuicao;
                            75 : CST_PIS := pisOperAquiPorST;
                            98 : CST_PIS := pisOutrasOperacoesEntrada;
                            99 : CST_PIS := pisOutrasOperacoes;
                          end;}
                          ...
                          {case StrToInt(aSitTribCofins) of
                            01 : CST_COFINS := cofinsValorAliquotaNormal;
                            02 : CST_COFINS := cofinsValorAliquotaDiferenciada;
                            03 : CST_COFINS := cofinsQtdeAliquotaUnidade;
                            04 : CST_COFINS := cofinsMonofaticaAliquotaZero;
                            05 : CST_COFINS := cofinsValorAliquotaPorST;
                            06 : CST_COFINS := cofinsAliquotaZero;
                            07 : CST_COFINS := cofinsIsentaContribuicao;
                            08 : CST_COFINS := cofinsSemIncidenciaContribuicao;
                            09 : CST_COFINS := cofinsSuspensaoContribuicao;
                            49 : CST_COFINS := cofinsOutrasOperacoesSaida;
                            50 : CST_COFINS := cofinsOperCredExcRecTribMercInt;
                            51 : CST_COFINS := cofinsOperCredExcRecNaoTribMercInt;
                            52 : CST_COFINS := cofinsOperCredExcRecExportacao;
                            53 : CST_COFINS := cofinsOperCredRecTribNaoTribMercInt;
                            54 : CST_COFINS := cofinsOperCredRecTribMercIntEExportacao;
                            55 : CST_COFINS := cofinsOperCredRecNaoTribMercIntEExportacao;
                            56 : CST_COFINS := cofinsOperCredRecTribENaoTribMercIntEExportacao;
                            60 : CST_COFINS := cofinsCredPresAquiExcRecTribMercInt;
                            61 : CST_COFINS := cofinsCredPresAquiExcRecNaoTribMercInt;
                            62 : CST_COFINS := cofinsCredPresAquiExcExcRecExportacao;
                            63 : CST_COFINS := cofinsCredPresAquiRecTribNaoTribMercInt;
                            64 : CST_COFINS := cofinsCredPresAquiRecTribMercIntEExportacao;
                            65 : CST_COFINS := cofinsCredPresAquiRecNaoTribMercIntEExportacao;
                            66 : CST_COFINS := cofinsCredPresAquiRecTribENaoTribMercIntEExportacao;
                            70 : CST_COFINS := cofinsOperAquiSemDirCredito;
                            71 : CST_COFINS := cofinsOperAquiComIsensao;
                            72 : CST_COFINS := cofinsOperAquiComSuspensao;
                            73 : CST_COFINS := cofinsOperAquiAliquotaZero;
                            74 : CST_COFINS := cofinsOperAqui_SemIncidenciaContribuicao;
                            75 : CST_COFINS := cofinsOperAquiPorST;
                            98 : CST_COFINS := cofinsOutrasOperacoesEntrada;
                            99 : CST_COFINS := cofinsOutrasOperacoes;
                          end;}
                   ...
                   begin
                     ...
                     begin
                        ...
                        AuxValor := AuxValor + QTrib.FieldByName('val_base_icms_sub').AsFloat;
                        if AuxFrete > 0 then
                        begin
...
//                 end;
         ...
         Q460.SQL.Add('p.valor_pis,p.valor_cofins,p.cnpjcpf,');
...
var Qry110, Qry125, Qry130, Qry140 : TFDQuery;
    aItem : Integer;
begin
   Qry110 := TFDQuery.Create(nil);
   Qry125 := TFDQuery.Create(nil);
   Qry130 := TFDQuery.Create(nil);
   Qry140 := TFDQuery.Create(nil);
   try
     Qry110.Connection := ConexaoDados;
     Qry125.Connection := ConexaoDados;
     Qry130.Connection := ConexaoDados;
     Qry140.Connection := ConexaoDados;
