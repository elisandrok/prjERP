...
procedure TFrmLancItensCompra.DbIDProdutoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  TabEnter(Self, Key);
...
begin
  ...
  begin
    ...
    begin
      DmWorkCom.RetornaCSTCFOPItemCompra(CST, CFOP, ALIQUOTA, REDUCAOBASE, UF_Fornecedor, DataItens.DataSet.FieldByName('IDPRODUTO').AsInteger);
...
begin
  inherited;
  aTotalItem := 0;
  aTotalItem := DataItens.DataSet.FieldByName('QUANTIDADE').AsFloat * DataItens.DataSet.FieldByName('VALOR').AsFloat +
    DataItens.DataSet.FieldByName('ACRESCIMO').AsFloat - DataItens.DataSet.FieldByName('DESCONTO').AsFloat;
...
var
  aValorICMS, aBaseCalculo: Double;
begin
  inherited;
