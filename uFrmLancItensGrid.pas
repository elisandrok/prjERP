...
procedure TFrmLancItensGrid.ComboGrupoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  TabEnter(Self, Key);
...
procedure TFrmLancItensGrid.GridCompItemKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if (Key = #13) then
  begin
...
procedure TFrmLancItensGrid.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TFrmLancItensGrid.DataListaProdutosDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  DmProdutos.Dados_ProdComposto(DataItens.DataSet.FieldByName('IDPRODUTO').AsInteger);
  ListaDetalhes(0);
...
var
  aIndice, aTotalM2, aValorM2: Double;
  aQuantidade: Double;
begin
  inherited;
  aQuantidade := 0;
  ...
  begin
    ...
    aTotalM2 := (DataCompItem.DataSet.FieldByName('QUANT_VIGAS').AsFloat * aIndice) * DataCompItem.DataSet.FieldByName('COMPRIMENTO_VIGA').AsFloat;
    DataCompItem.DataSet.FieldByName('TOTAL_M2').AsFloat := TruncaValor(aTotalM2, 2);
    aValorM2 := (DataCompItem.DataSet.FieldByName('TOTAL_M2').AsFloat * DataItens.DataSet.FieldByName('VALOR').AsCurrency);
