...
uses
  ...
  Datasnap.Provider, Data.DB, Vcl.ExtCtrls, Vcl.ComCtrls, FireDAC.Stan.Intf,
...
uses
  ufrmDetalhaCupom, uLibrary, uDmConexao;
...
begin
  ...
  begin
    ...
    begin
      Close;
      CommandText := 'select i.idproduto,i.descricao,i.quantidade,i.valor, ' + '       i.desconto,i.valor_total ' + 'from pedido_itens i ' +
...
begin
  ...
  begin
    ...
    begin
      Close;
      CommandText := 'select f.descricao,sum(f.valor) as valor ' + 'from pedido p ' + 'inner join pedido_formapag f on p.id = f.idpedido ' +
        'where p.idempresa = ' + CdsTotais.FieldByName('ID_EMPRESA').AsString + ' ' + 'and p.caixa = ' + QuotedStr(CdsTotais.FieldByName('CAIXA').AsString) +
        ' ' + 'and p.data_entrega = current_date ' + 'group by f.descricao';
      Open;
    end;
    //
    with CdsCupons do
    begin
      Close;
      CommandText := 'select c.id,c.coo,c.data_entrega,c.cliente,c.valor_total ' + 'from pedido c ' + 'where c.idempresa = ' +
        CdsTotais.FieldByName('ID_EMPRESA').AsString + ' ' + 'and c.caixa = ' + QuotedStr(CdsTotais.FieldByName('CAIXA').AsString) + ' ' +
