...
begin
  ...
  if CdsConsulta.FieldByName('STATUS').AsInteger = 1 then
  begin
    MessageDlg('Esta conta já está quitada no sistema.' + #13 + #10 + 'Para excluir, use a opção de remover a quitação!!!', mtWarning, [mbOK], 0);
    Exit;
  end;
  //
  if DataConsulta.DataSet.RecordCount > 0 then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MessageDlg('É foi localizado a conta a ser retirada a quitação!!!', mtWarning, [mbOK], 0);
    end
    else
      MessageDlg('Esta conta não está quitada no sistema. Verifique!!!', mtWarning, [mbOK], 0);
  end
  else
    MessageDlg('É necessário selecionar a conta para remover a quitação!!!', mtWarning, [mbOK], 0);
