...
begin
  ExibeMensagem(0, 'Confirma a Transação');
  result := false;
  cConteudo := '';
  cLinhaArquivo := '';
  cConteudo := '';
  ...
  begin
    ...
    begin
      RestauraAplicativo;
      MessageDlg('Gerenciador padrão desativado e será ativado automaticamente!', mtWarning, [mbOK], 0);
...
var
  ...
  cArquivo: TextFile;
begin
  ExibeMensagem(0, 'Não confirma a transação');
  // zera variáveis
  gNSU := '';
  gValor := '';
  gNomeRede := '';
  ...
  begin
    ...
    while not FileExists(gPathRespTEF + 'INTPOS.STS') do
      Sleep(1000);
    DeleteFile(gPathRespTEF + 'INTPOS.STS');
    // carrega informações da mensagem de NSU
    if Trim(gValor) <> '' then
    begin
    ...
    if sTipoTef in [0, 2] then
    begin
      AuxMensagem := 'Cancelada a Transação' + #13 + #13 + 'Rede:   ' + gNomeRede + #13;
      if gNSU <> '' then
    ...
    begin
      if pExibeMensagemTef then
        MessageDlg('Transação nao efetuada. Favor reter o cupom!!!', mtWarning, [mbOK], 0);
    end;
  end;
  // limpa informação de campo NSU
  gNSU := '';
  // apaga arquivo
...
var
  ...
  aVias: Integer;
begin
  ExibeMensagem(0, 'Cancelamento de transação.');
  result := false;
  // zera variáveis
  gNSU := '';
  gValor := '';
...
begin
  ...
  begin
    ...
    MessageDlg('Erro ao comunicar com o Gerenciador Padão!', mtWarning, [mbOK], 0);
  CloseFile(cArquivo);
...
begin
  ...
  try
    ...
    MessageDlg('Erro na comunicação com o ECF!!', mtError, [mbOK], 0);
  end;
...
begin
  ...
  begin
    Mensagem := 'Arquivo de configuração não encontrado. Verifique!';
    result := false;
  end
  else
  begin
    ...
    begin
      Mensagem := 'Serial do ECF não confere. Verifique!';
      result := false;
    end
    else
    begin
      ...
      begin
        { Quanto o GT estiver diferente deve-se:
          * Caso o CRO da ultima redução gravada estiver menor ou igual ao da ECF, e
          o CRZ e Venda Bruta estiver igual, recupera os dados;
          * Caso o CRO da ultima redução Z gravada esteja maior que o da ECF,
          deve barrar;
          * Caso o CRZ ou Venda bruta da ultima redução Z gravada estiver
        ...
        begin
          Mensagem := 'O GT, CRO, CRZ ou Venda bruta não confere. Verifique!';
          result := false;
        end
        else
          AtualizaINI(false);
      ...
      begin
        ...
        begin
          Mensagem := 'A Data ECF não confere. Verifique!';
          // Result := false;
        end
        else
        begin
          ...
          begin
            Mensagem := 'A hora do ECF não confere. Verifique!';
            // Result := false;
          end;
        end;
      end;
    end;
    // verifica configuração de bicos
    if (gPosto) and (result) then
    begin
      if sCdsCtrlBicos.IsEmpty then
      begin
        Mensagem := 'Não foi detectado a configuração dos bicos. Verifique!';
        result := false;
      end;
      if result then
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Mensagem := 'Bomba número ' + sCdsCtrlBicos.FieldByName('BOMBA').AsString + ', Não encontrada no cadastro. Verifique!';
              result := false;
            end;
            if not LocalizaDadosBico(sCdsCtrlBicos.FieldByName('BICO').AsString) then
            begin
              Mensagem := 'Bico número ' + sCdsCtrlBicos.FieldByName('BICO').AsString + ', Não encontrada no cadastro. Verifique!';
              result := false;
            end;
            if not LocalizaDadosTanque(sCdsCtrlBicos.FieldByName('TANQUE').AsInteger) then
            begin
              Mensagem := 'Tanque número ' + sCdsCtrlBicos.FieldByName('TANQUE').AsString + ', Não encontrada no cadastro. Verifique!';
              result := false;
            end;
            Next;
          end;
...
begin
  ...
  begin
    ...
    // verifica o tipo de TEF e faz a impressão de acordo
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // exibe mensagem
          ExibeMensagem(5, 'Por favor, destaque a 1ª Via');
          ExibeMensagem(0, 'Imprimindo Cupom TEF');
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // Opções de impressão da segunda via
...
begin
  ...
  try
    ...
    begin
      MessageDlg('É necessário configurar uma impressora!', mtInformation, [mbOK], 0);
      Application.Terminate;
    end;
    DmAcBr.ACBrECF1.Modelo := TACBrECFModelo(gModeloECF);
    DmAcBr.ACBrECF1.Porta := gPortaECF;
    // caso não seja Sweda não passa os parametros abaixo
...
begin
  try
    ...
    MessageDlg('Erro de cominucação com a impressora!!!', mtError, [mbOK], 0);
  end;
...
begin
  try
    ...
    ShowMessage('Falha de comunicação com ECF!');
  end;
...
begin
  ...
  try
    ...
    Mensagem := 'Falha na comunicação com o ECF';
    result := false;
  end;
...
begin
  ...
  begin
    MessageDlg('Para emitir a redução do dia, use a opção para fechar a Jornada Fiscal!', mtWarning, [mbOK], 0);
    Abort;
  end;
  try
...
begin
  ...
  begin
    // carrega informações de path da bandeira
    CarregarDadosPathTEF;
    // realiza a não confirmação
    if sTipoTef in [0, 2] then // discado
    begin
  ...
  begin
    ...
    begin
      ...
      if aNSU <> '' then
      begin
        MessageDlg('Transação TEF efetuada. Favor re-imprimir último cupom.' + #13 + #10 + 'NSU(s): ' + copy(aNSU, 1, Length(aNSU) - 1) + #13 + #10 +
          'Para Rede Cielo utilizar últimos 6 digitos', mtWarning, [mbOK], 0)
      end;
    end;
  end
...
begin
  result := True;
  try
    // procura aplicação em execução
    Hwnd := FindWindow('TApplication', pChar(Application.Title));
    // Se aplicação não estiver em uso executa
    if Hwnd <> 0 then
    begin
...
begin
  ...
  try
    ...
    MessageDlg('Applicativo GP não encontrado!', mtWarning, [mbOK], 0);
    result := false;
  end;
...
begin
  ...
  ImprimeRelatorioGerencial('                                            ');
  ImprimeRelatorioGerencial('     RECONHEÇO E PAGAREI A DIVIDA ACIMA     ');
  ImprimeRelatorioGerencial('                                            ');
  FechaRelatorioGerencial;
...
begin
  ...
  ImprimeRelatorioGerencial('ESTOQUE FISICO DE COMBUSTÍVEIS');
...
begin
  ...
  ImprimeRelatorioGerencial('PERDA DE COMBUSTÍVEIS');
...
begin
  ...
  // se não abrir o vinculado abre o gerencial
  if not aOK then
    DmAcBr.ACBrECF1.AbreRelatorioGerencial();
  // imprime a autorização
  ImprimeRelatorioGerencial(Texto);
  // fecha o vinculado
  FechaRelatorioGerencial;
...
begin
  ...
  try
    ...
    // Abre o documento para impressão
    prn.OpenDoc('');
    ...
    prn.SendData('                                            ' + #13 + #10);
    prn.SendData('     RECONHEÇO E PAGAREI A DIVIDA ACIMA     ' + #13 + #10);
    prn.SendData('                                            ' + #13 + #10);
...
var
  ...
  cArquivo: TextFile;
begin
  { XXX } ExibeMensagem(0, 'Realizando Recarga de Pré pago.');
