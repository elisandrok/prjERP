...
var
  aValor, aValICMS: Double;
begin
  aValor := DataWork.DataSet.FieldByName('QUANTIDADE').AsFloat * DataWork.DataSet.FieldByName('VALOR').AsFloat;
...
procedure TFrmCadReg61.ActionList1Update(Action: TBasicAction; var Handled: Boolean);
begin
  inherited;
...
begin
  ...
  begin
    DmWorkCom.GravaRegistroA2(DataWork.DataSet.FieldByName('DATA_EMISSAO').AsDateTime, 'DINHEIRO', 3, DataWork.DataSet.FieldByName('TOTAL').AsFloat);
  end;
  inherited;
  ...
  begin
    ...
    DmPosto.RegistraAbastecimentos(aNumBico, FormatDateTime('dd/mm/yyyy', now), FormatDateTime('hh:mm:ss', now), aEncerranteAnterior,
      DataWork.DataSet.FieldByName('QUANTIDADE').AsFloat, DataWork.DataSet.FieldByName('VALOR').AsFloat, DataWork.DataSet.FieldByName('TOTAL').AsFloat, False,
      true, DataWork.DataSet.FieldByName('NUM_ORDEM').AsString);
  end;
