...
begin
  ...
  begin
    Close;
    Comando := 'select a.* from abastecimentos a ' + 'left outer join bicos_abastec bi on bi.bico_concentrador = a.num_bico ' +
      'left outer join bombas_abastec bo on bo.id = bi.id_bomba ' + 'where a.status_abastec = 1 ' + 'and a.idproduto = ' + IntToStr(sIDProduto);
    CommandText := Comando;
    Open;
  end;
...
procedure TFrmListaAbastec.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #27 then
  begin
