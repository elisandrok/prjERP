...
uses
  SysUtils, Classes, FMTBcd, DBClient, Provider, DB, Windows, Messages,
  Variants, Graphics, Controls, Forms, ExtCtrls, ComCtrls, ImgList, Dialogs,
  Menus, uFuncoesPAFECF, uDmAcBr, FireDAC.Stan.Intf, FireDAC.Stan.Option,
...
begin
  ...
  begin
    Close;
    CommandText := 'select * from nfe_xml x ' + 'where x.chave_nfe = ' + QuotedStr('NFe' + ChaveNFe);
    Open;
    Result := not IsEmpty;
  end;
...
begin
  ...
  begin
    Close;
    CommandText := 'delete from nfe_xml x ' + 'where x.chave_nfe = ' + QuotedStr('NFe' + ChaveNFe);
    Execute;
  end;
...
function TDmAuxiliar.AbreDadosRegistro60R(pDatai, pdataf: TDatetime; pNumSerie: String): Boolean;
begin
  try
    with CdsRegAuxiliar1 do
    begin
...
function TDmAuxiliar.RetornaIDA2Movimento(pData: TDatetime; Descricao: String; Tipo: Integer): Integer;
begin
  Result := 0;
  try
    with CdsGeral do
    begin
      Close;
      CommandText := 'select x.id from paf_registro_a2 x ' + 'where x.data_movimento = :pData ' + 'and x.descricao = :pDescricao ' + 'and x.cod_tipo = ' +
        IntToStr(Tipo);
...
begin
  ...
  begin
    Close;
    CommandText := 'select r.num_ordem,r.data_emissao,c.nome, ' + '       c.cnpj_cpf,sum(r.valor) as valor ' + 'from registro61 r ' +
      'inner join contato c on c.id = r.id_cliente ' + 'where r.data_emissao between :pDatai and :pdataf ' + 'and r.idempresa = :pEmpresa ' +
      'group by r.num_ordem,r.data_emissao,c.nome, ' + '         c.cnpj_cpf';
    Params.ParamByName('pDatai').AsDate := pDatai;
    Params.ParamByName('pDataf').AsDate := pdataf;
    Params.ParamByName('pEmpresa').AsInteger := sIDEmpresa;
    Open;
...
function TDmAuxiliar.AbreDadosReg61R5(pDatai, pdataf: TDatetime): Boolean;
begin
  with CdsGeral do
  begin
    Close;
    CommandText := 'select r.num_ordem,r.idproduto,r.descricao, ' + '       r.quantidade,p.unidade,r.valor, ' + '       r.total,r.cst,r.aliq_icms ' +
      'from registro61 r ' + 'inner join produtos p on r.idproduto = p.id ' + 'where r.data_emissao between :pdatai and :pdataf ' +
