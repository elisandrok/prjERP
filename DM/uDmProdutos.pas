...
type
  TDmProdutos = class(TDataModule)
    ...
    QryGrupoGRUPO_ID: TIntegerField;
    QryGrupoGRUPO_DESCRICAO: TStringField;
    QryGrupoATIVO: TStringField;
    QryGrupoATIVO_VENDA: TStringField;
    CdsGrupoGRUPO_ID: TIntegerField;
    CdsGrupoGRUPO_DESCRICAO: TStringField;
...
begin
  Dados_SubGrupo(CdsGrupo.FieldByName('GRUPO_ID').AsInteger);

end;

procedure TDmProdutos.CdsGrupoNewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('GRUPO_ID').AsInteger := GenID('GRUPO');
...
begin
  ...
  begin
    ...
    begin
      MessageDlg('SoftWare de Demonstração!' + #13 + #10 + 'Não é permitido ultrapassar o limite de produtos estabelecido,' + #13 + #10 + 'Máximo ' +
        IntToStr(pNumProd) + '. Entre em contato com o suporte técnico!!!', mtWarning, [mbOK], 0);
      Abort;
    end;
  end;
...
begin
  ...
  DataSet.FieldByName('IDGRUPO').AsInteger := CdsGrupo.FieldByName('GRUPO_ID').AsInteger;
  DataSet.FieldByName('ATIVO').AsString := 'T';
...
begin
  ...
  begin
    MessageDlg('É necessário efetuar a gravação do fechamento inicial!!!', mtWarning, [mbOK], 0);
    Result := False;
    Abort;
  end
  else
...
begin
  { if not DmConexao.ConectaBancoSISCOM then
    begin
    MessageDlg('Não foi possível estabelecer uma conexão com o Banco de dados V15. Verifique!!!',mtWarning,[mbOK],0);
    Exit;
    end;
    //
    try
